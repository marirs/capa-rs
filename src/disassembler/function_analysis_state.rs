...
pub struct FunctionAnalysisState {
    pub start_addr: u64,
    pub block_queue: std::collections::VecDeque<u64>,
    ...
    pub instruction_start_bytes: std::collections::HashSet<u64>,
    processed_blocks: std::collections::HashSet<u64>,
    processed_bytes: std::collections::HashSet<u64>,
...
impl FunctionAnalysisState {
    pub fn new(addr: u64) -> Result<FunctionAnalysisState> {
        Ok(FunctionAnalysisState {
            ...
            instructions: vec![],
            instruction_start_bytes: std::collections::HashSet::new(),
            processed_blocks: std::collections::HashSet::new(),
            processed_bytes: std::collections::HashSet::new(),
            jump_targets: vec![],
    ...
    pub fn has_unprocessed_blocks(&self) -> bool {
        let ss: std::collections::HashSet<u64> = self.block_queue.clone().into_iter().collect();
    ...
    pub fn finalize_analysis(
        &mut self,
        as_gap: bool,
        disassembly: &mut crate::disassembler::DisassemblyResult,
    ) -> Result<bool> {
        if as_gap {
        ...
        for start in &potential_starts {
            ...
            for i in ins[&start]..self.instructions.len() {
                ...
                if self.code_refs_from.contains_key(&current.0) {
                    if !crate::disassembler::CALL_INS
                        .contains(&Some(current.2.as_ref().unwrap().as_str()))
    ...
    pub fn finalize_regular_analysis(
        &mut self,
        disassembly: &mut crate::disassembler::DisassemblyResult,
    ) -> Result<()> {
    ...
    pub fn identify_call_conflicts(
        &self,
        all_refs: &std::collections::HashMap<u64, u64>,
    ) -> Result<std::collections::HashMap<u64, Vec<u64>>> {
        let mut conflicts: std::collections::HashMap<u64, Vec<u64>> =
            std::collections::HashMap::new();
        let non_instruction_start_bytes: std::collections::HashSet<u64> = self
            .processed_bytes
            .difference(&self.instruction_start_bytes)
            .map(|e| *e)
            .collect();
        let all_refs_set: std::collections::HashSet<u64> = all_refs.keys().map(|e| *e).collect();
