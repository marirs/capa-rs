...
use crate::disassembler::function_candidate::FunctionCandidate;
...
#[derive(Debug)]
struct GapSequences {
    gs: std::collections::HashMap<usize, Vec<Vec<u8>>>,
}

impl GapSequences {
    pub fn new() -> GapSequences {
        let mut gs = GapSequences {
            gs: std::collections::HashMap::new(),
        };
        gs.gs.insert(
            1,
            vec![
...
pub struct FunctionCandidateManager {
    ...
    all_call_refs: std::collections::HashMap<u64, u64>,
    pub symbol_addresses: Vec<u64>,
    pub candidates: std::collections::HashMap<u64, FunctionCandidate>,
...
impl FunctionCandidateManager {
    pub fn new() -> FunctionCandidateManager {
        FunctionCandidateManager {
            ...
            code_areas: vec![],
            all_call_refs: std::collections::HashMap::new(),
            symbol_addresses: vec![],
            candidates: std::collections::HashMap::<u64, FunctionCandidate>::new(),
    ...
    pub fn update_candidates(
        &mut self,
        state: &crate::disassembler::FunctionAnalysisState,
    ) -> Result<()> {
        if let Ok(_s) = std::env::var("CAPARS_HIGH_ACCURACY") {
