...
pub struct DisassemblyResult {
    ...
    apis: std::collections::HashMap<u64, crate::disassembler::label_providers::ApiEntry>,
...
impl Disassembler {
    ...
    pub fn disassemble_file(file_name: &str) -> Result<DisassemblyReport> {
        ...
        disassembler.analyse_buffer(binary_info)?;
    ...
    pub fn analyse_buffer(&mut self, bi: BinaryInfo) -> Result<&DisassemblyResult> {
        //LOGGER.debug("Analyzing buffer with %d bytes @0x%08x",
        // binary_info.binary_size, binary_info.base_addr)
        self.update_label_providers(&bi)?;
        self.disassembly.init(bi)?;
        ...
        let mut state = None;
        for addr in queue {
            state = match self.analyse_function(addr, false) {
                Ok(s) => Some(s),
                Err(_) => None,
            }
        }
            ...
            .next_gap_candidate(Some(next_gap), &self.disassembly)
        {
            ...
            state = match self.analyse_function(gap_candidate, true) {
                Ok(s) => {
        ...
        if let Ok(_) = std::env::var("CAPARS_RESOLVE_TAILCALLS") {
            let tailcalled_functions =
                TailCallAnalyser::resolve_tailcalls(self, &mut state.unwrap())?;
    ...
    fn analyse_function(&mut self, start_addr: u64, as_gap: bool) -> Result<FunctionAnalysisState> {
        self.tailcall_analyzer.init()?;
        let mut _i = 0;
    ...
    fn update_api_information(
        ...
        let mut api_entry = crate::disassembler::label_providers::ApiEntry {
